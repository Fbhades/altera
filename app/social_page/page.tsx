...
export default function SocialPage() {
    ...
    if (user?.primaryEmailAddress?.emailAddress) {
      try {
        const response = await fetch(
          `/api/auth/${user.primaryEmailAddress.emailAddress}`
        );
  ...
  const fetchFollowedUsersPosts = async (userId: number) => {
    try {
      ...
      const data = await response.json();
      console.log("Following list:", data.followers); // Debug log
      const followedUserIds = data.followers.map(String);
      console.log("Followed users:", followedUserIds); // Debug log

      const postsPromises = followedUserIds.map(async (followedUserId: string) => {
        console.log("Fetching posts for user", followedUserId); // Debug log
    ...
    try {
      if (searchTerm.trim()) {
        ...
        const filteredUsers = usersWithIds.filter(
          (user: UserInterface) => user.id !== dbUserId
        );
            ...
            {searchResults.map((result) => (
              <Card key={result.id} className="p-4">
                <div className="flex justify-between items-center">
                  ...
                  <Button
                    onClick={() => 
                      following.includes(result.id.toString())
